...
const CarouselImages = ({ carouselImages, onAllCarouselImagesLoaded }: CarouselImageProps) => {
      ...
      <div className={showCarousel ? 'visible' : 'hidden'}>
          <Carousel
              ...
              items={carouselImages?.map((carouselImage, index) => {
                  return {
                      item: <ImageTile key={index} image={carouselImage} onImageLoaded={(imageLoader) => {
                          setCarouselImagesLoaded([...carouselImagesLoaded, ...[imageLoader]]);
                      }} />,
                  };
              })}
          />
